terraform {
  ...
  backend "s3" {
    ...
    key                         = "deployments/screening---loan-origination--cards-/terraform.tfstate"
    region                      = "us-east-1"
...
resource "digitalocean_project" "main" {
  name        = "ab-screening---loan-origination--cards-"
  description = "AgentBridge: Screening • Loan Origination (Cards)"
...
resource "digitalocean_vpc" "main" {
  name        = "ab-screening---loan-origination--cards--vpc"
  region      = var.region
  ip_range    = var.vpc_cidr
  description = "Isolated VPC for AgentBridge Screening • Loan Origination (Cards)"
}
...
resource "digitalocean_ssh_key" "deploy" {
  ...
  name       = "ab-screening---loan-origination--cards--deploy"
...
resource "digitalocean_firewall" "main" {
  name = "ab-screening---loan-origination--cards--fw"
  ...
  # n8n editor — restricted to operator CIDRs (NOT public)
  inbound_rule {
    protocol         = "tcp"
    port_range       = "5678"
    source_addresses = var.allowed_ssh_cidrs
...
resource "digitalocean_volume" "data" {
  ...
  region                  = var.region
  name                    = "ab-screening---loan-origination--cards--data"
  size                    = var.additional_storage_gb
  initial_filesystem_type = "ext4"
  description             = "Persistent data for AgentBridge Screening • Loan Origination (Cards)"

  tags = ["agentbridge", "blueprint-screening---loan-origination--cards-", "data"]
}
...
resource "digitalocean_spaces_bucket" "artifacts" {
  ...
  name   = "ab-screening---loan-origination--cards--artifacts"
...
resource "digitalocean_loadbalancer" "main" {
  ...
  name   = "ab-screening---loan-origination--cards--lb"
  region = var.region
  vpc_uuid = digitalocean_vpc.main.id
...
resource "digitalocean_monitor_alert" "cpu_high" {
  ...
  description = "AgentBridge screening---loan-origination--cards-: CPU > 85% for 5 min"
}
...
resource "digitalocean_monitor_alert" "memory_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge screening---loan-origination--cards-: Memory > 90% for 5 min"
}
...
resource "digitalocean_monitor_alert" "disk_high" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.main.id]
  description = "AgentBridge screening---loan-origination--cards-: Disk > 85%"
}
